...
simulate_gene_trees_es <- function(species_tree, num_trees = 100, hybridization_events = NULL) {
  species_tree$node.label <- NULL
  if(any(branching.times(species_tree)<0)) {
    species_tree <- make_ultrametric(species_tree)
  }
  ...
  # Add migration settings if provided
  # migration_command <- ""
  # if (!is.null(hybridization_matrix)) {
  #   migration_command <- paste0("-es ",paste(apply(migration_matrix, 1, paste, collapse=" "), collapse=" -em "))
  # }
  
  hybrid_commands <- character()
  if (!is.null(hybridization_events)) {
    new_pop_id <- num_tips
    for (i in seq_len(nrow(hybridization_events))) {
      ev <- hybridization_events[i, ]
      if(i==1){
        hybrid_commands <- sprintf("-es %f %d %f", ev$time, ev$recipient, ev$frac)
      }
      else{
        hybrid_commands <- paste(hybrid_commands,
                                 sprintf("-es %f %d %f", ev$time, ev$recipient, ev$frac))
      }
      
      new_pop_id <- new_pop_id + 1
      # Merge donor with the new pop (same time)
      hybrid_commands <- paste(hybrid_commands,
                           sprintf("-ej %f %d %d", ev$time, new_pop_id, ev$donor))
    }
  }
  
  # Construct ms command
  ms_opts <- paste("-T -I", num_tips, paste(rep(1, num_tips), collapse=" "), paste(ms_commands, collapse=" "), hybrid_commands)
  print(ms_opts)
  ms_out <- ms(nsam=num_tips, nreps=num_trees, opts=ms_opts)
  trees <- get_trees_from_ms(ms_out)
